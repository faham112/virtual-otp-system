    ...
    try {
      ...
      if (
        ...
        try {
          ...
          const next = pickNextInStock(country, map);
          if (next) {
            const label = COUNTRIES.find((c) => c.value === next)?.label || next;
            setCountry(next);
            setHint(`No phones for previous choice — switched to ${label}. Tap Buy again.`);
          } else if (country !== "any") {
            setCountry("any");
            setHint("No stock on selected countries — switched to Any (Cheapest). Tap Buy again.");
          } else {
            setHint("No free phones on 5sim right now for this service. Try another service or wait.");
          }
        } catch {
          setHint("Stock refresh failed. Try another country manually.");
        }
      }
    } finally {
      setLoading(false);
    ...
    <div className="min-h-screen">
      ...
      <main className="max-w-lg mx-auto px-4 py-8">
        <div className="card p-6 space-y-6">
          ...
          {hint && (
            <div className="bg-blue-500/10 border border-blue-500/30 text-blue-300 p-3.5 rounded-xl text-sm">
              {hint}
            </div>
          )}
          ...
          {success ? (
            ...
            <>
              ...
              <p className="text-xs text-center text-gray-500">
                Full refund if OTP fails or times out. On no free phones we auto-switch to next in-stock country.
              </p>
            </>
          )}
        </div>
