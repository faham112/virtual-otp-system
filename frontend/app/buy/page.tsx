...
export default function BuyPage() {
  ...
  const sortedRows = useMemo(() => {
    ...
    return [...rows].sort((a, b) => {
      const ap = Number(a.user_price || 0);
      const bp = Number(b.user_price || 0);
      const aOk = !!a.available && ap > 0;
      const bOk = !!b.available && bp > 0;
  ...
  const shownPrice = Number(quote?.user_price || selectedRow?.user_price || 0);
  const priceLabel = (row: any) => {
    const p = Number(row?.user_price || 0);
            ...
            <>
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {SERVICES.map((s) => (
                    <button key={s.value} type="button" onClick={() => { setService(s.value); setError(""); }}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Country {stockLoading ? "(loading rates...)" : `(${sortedRows.length} \u00b7 cheap first)`}</label>
                ...
                <button type="button" onClick={() => { setCountry("any"); setError(""); }}
                  ...
                  <span className="text-xl">\uD83C\uDF10</span><span className="flex-1">Any (Cheapest in stock)</span>
                </button>
                  ...
                  {sortedRows.map((row) => {
                    ...
                    return (
                      <button key={row.country} type="button" onClick={() => { setCountry(row.country); setError(""); }}
                        ...
                        <span className={`text-xs font-semibold ${row.available && p > 0 ? "text-emerald-400" : "text-amber-400"}`}>
                          {row.available && p > 0 ? priceLabel(row) : p > 0 ? `${priceLabel(row)} \u00b7 OOS` : "Out of stock"}
                        </span>
                      </button>
                    );
                  })}
              ...
              <div className="bg-[#12151c] rounded-xl p-4 border border-[#2a2f3d] space-y-2">
                ...
                {priceLoading ? <p className="text-sm text-gray-400">Fetching price...</p> : (quote?.available || shownPrice > 0) ? (
                  <div className="flex items-center gap-3">
                    <span className="text-2xl font-bold text-emerald-400">${Number(quote?.user_price || shownPrice).toFixed(4)}</span>
              ...
              <button onClick={handleBuy} disabled={loading || priceLoading || (quote && !quote.available)} className="w-full btn-primary py-3.5 text-base">
                {loading ? "Buying number..." : quote?.available ? `Buy for $${Number(quote.user_price).toFixed(4)}` : "Out of stock"}
              </button>
            </>
          )}
        </div>
